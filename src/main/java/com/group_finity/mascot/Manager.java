...
public class Manager {
    ...
    public void setBehaviorAll(final String name) {
        synchronized (this.getMascots()) {
            for (final Mascot mascot : this.getMascots()) {
                try {
                    mascot.setBehavior(com.group_finity.mascot.Main.getInstance().getConfiguration(mascot.getImageSet()).buildBehavior(name));
                } catch (final BehaviorInstantiationException e) {
                    log.log(Level.SEVERE, "Failed to initialize the following actions", e);
                    com.group_finity.mascot.Main.showError(com.group_finity.mascot.Main.getInstance().getLanguageBundle().getString("FailedSetBehaviourErrorMessage") + "\n" + e.getMessage() + "\n" + com.group_finity.mascot.Main.getInstance().getLanguageBundle().getString("SeeLogForDetails"));
                    mascot.dispose();
